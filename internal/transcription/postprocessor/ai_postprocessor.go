...
func (p *AITextPostprocessor) processBatch(
	...
	cleaned, err := parseCleanupResponse(content, len(inputSegments))
	if err != nil {
...
func parseCleanupResponse(content string, expectedCount int) ([]CleanedSegment, error) {
	...
	// Validate segment count matches (LLM shouldn't add/remove segments)
	if len(segments) != expectedCount {
		return nil, fmt.Errorf("segment count mismatch: expected %d, got %d", expectedCount, len(segments))
	}
